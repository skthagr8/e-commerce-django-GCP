...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('csvs/product_types.csv', type=str, help='The path to the CSV file to import')

    def handle(self, *args, **kwargs):
        csv_filepath = kwargs['csvs/product_types.csv']
        ...
        try:
            with open(csv_filepath, newline='', encoding='utf-8') as csvfile:
                ...
                for row in reader:
                    try:
                        type = row['Name']
                        product_type, created = ProductType.objects.get_or_create(name=type)
                        if created:
                            count += 1
